...
export default function ProductAttributesInput({ attributes, onChange }: ProductAttributesInputProps) {
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className={`inline-flex items-center px-3 py-1.5 border border-transparent text-sm font-medium rounded-md shadow-sm text-white ${attributes.length >= MAX_ATTRIBUTES ? 'bg-gray-400 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700'
            ...
            {attributes.length === 0 && (
                <p className="text-sm text-gray-500 italic">No attributes added yet. Add attributes like "Color" or "Size" to generate variants.</p>
            )}
                    ...
                    <div key={attribute.id} className="relative bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 gap-y-4 gap-x-4 sm:grid-cols-6">
                            <div className="sm:col-span-2">
                                <label htmlFor={`attr-name-${attribute.id}`} className="block text-sm font-medium text-gray-700">
                                    Attribute Name
                                </label>
                                <div className="mt-1">
                                    <input
                            ...
                            <div className="sm:col-span-4">
                                <CreatableMultiSelect
                                    ...
                                    onChange={(newValues) => handleAttributeValuesChange(attribute.id, newValues)}
